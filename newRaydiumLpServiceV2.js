...
const { Api } = require("@raydium-io/raydium-sdk");
...
async function addSerumAddresses(tokenData, accounts, accountIndices) {
    try {
        const serumKeys = {
            marketProgramId: new PublicKey(accounts[accountIndices[15]]).toString(),
            marketId: new PublicKey(accounts[accountIndices[16]]).toString(),
            marketBaseVault: new PublicKey(accounts[accountIndices[18]]).toString(),
            marketQuoteVault: new PublicKey(accounts[accountIndices[19]]).toString(),
            marketAuthority: new PublicKey(accounts[accountIndices[20]]).toString(),
            marketEventQueue: new PublicKey(accounts[accountIndices[21]]).toString(),
            marketBids: new PublicKey(accounts[accountIndices[22]]).toString(),
            marketAsks: new PublicKey(accounts[accountIndices[23]]).toString(),
        };
        ...
        const poolKeys = await api.fetchPoolKeysById({
            idList: [tokenData.ammId]
        });

        if (poolKeys && poolKeys.length > 0) {
            const poolData = poolKeys[0];
            Object.assign(serumKeys, {
                // Add any additional fields from poolData.serumMarket if needed
            });
        }
